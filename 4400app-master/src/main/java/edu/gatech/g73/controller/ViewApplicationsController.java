...
public class ViewApplicationsController implements FXController<String, String> {
    ...
    private void accept() {
        try {
          ...
          PROJ_INPUT = (String)(((ObservableList)selectedItems.get(0)).get(0));
          NAME_INPUT = (String)(((ObservableList)selectedItems.get(0)).get(1));
          Connection connect = new SQLConnector().connect();
          Statement stmt = connect.createStatement();
          String acceptProject = String.format("update Apply set status = 'A' where Apply.Project_name = '%s' and Apply.Student_name = '%s';", PROJ_INPUT, NAME_INPUT);

                      stmt.executeUpdate(acceptProject);
    ...
    private void reject() {
        try {
          ...
          NAME_INPUT = (String)(((ObservableList)selectedItems.get(0)).get(1));
            ...
            String rejectProject = String.format("update Apply set status = 'R' where Apply.Project_name = '%s' and Apply.Student_name = '%s';", PROJ_INPUT, NAME_INPUT);
            stmt.executeUpdate(rejectProject);
